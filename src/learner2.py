...
class Learner(GetAttr):
    ...
    def set_opt(self):
        if self.model:
            ...
            self.opt = self.opt_func(params, self.lr)
        else:
            self.opt = None
...
class PlottingCallback(Callback):
    ...
    def after_epoch(self):
        ...
        train_loss = self.learner.loss.item()  # Current training loss
        val_loss = getattr(self.learner, 'valid_loss', None)  # Validation loss if available
    ...
    def _plot(self):
        ...
        self.ax1.set_title('Training Loss vs Epochs')
        ...
        if self.val_losses:
            self.ax2.plot(self.epochs, self.val_losses, 'r-', label='Validation Loss')
            self.ax2.set_title('Validation Loss vs Epochs')
        ...
        plt.savefig(f'{self.save_dir}/training_plot_{timestamp}.png')
        plt.close()
